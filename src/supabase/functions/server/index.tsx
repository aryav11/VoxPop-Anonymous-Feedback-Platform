...
function analyzeText(
  ...
  if (!category) {
    if (
      ...
      lowercaseText.includes("cafeteria") ||
      lowercaseText.includes("meal")
    ) {
      detectedTopic = "Dining";
    } else if (
      lowercaseText.includes("dorm") ||
      lowercaseText.includes("housing") ||
      lowercaseText.includes("room") ||
      lowercaseText.includes("residence")
    ) {
      detectedTopic = "Housing";
    } else if (
      lowercaseText.includes("wifi") ||
      lowercaseText.includes("internet") ||
      lowercaseText.includes("computer") ||
      lowercaseText.includes("technology")
    ) {
      detectedTopic = "IT/Technology";
    } else if (
      lowercaseText.includes("stress") ||
      lowercaseText.includes("anxiety") ||
      lowercaseText.includes("mental") ||
      lowercaseText.includes("counseling")
    ) {
      detectedTopic = "Mental Health";
    } else if (
      lowercaseText.includes("parking") ||
      lowercaseText.includes("bus") ||
      lowercaseText.includes("transport") ||
      lowercaseText.includes("traffic")
    ) {
...
app.post("/make-server-2459992a/feedback", async (c) => {
  try {
    ...
    const existingTrend = (await kv.get(trendsKey)) || {
      count: 0,
...
// Get session data
app.get(
  "/make-server-2459992a/session/:sessionId",
  async (c) => {
    try {
      ...
      const messages =
        (await kv.get(`messages:${sessionId}`)) || [];
...
app.post(
  "/make-server-2459992a/session/:sessionId/message",
  async (c) => {
    try {
      const sessionId = c.req.param("sessionId");
      const body = await c.req.json();
...
app.post(
  "/make-server-2459992a/community/:postId/upvote",
  async (c) => {
    try {
...
app.put(
  "/make-server-2459992a/admin/feedback/:sessionId/status",
  async (c) => {
    try {
      const sessionId = c.req.param("sessionId");
      const body = await c.req.json();
...
app.get("/make-server-2459992a/progress", async (c) => {
  try {
    ...
    return c.json({
      issuesByStatus,
      categories: [
        ...new Set(allFeedback.map((f) => f.category)),
      ],
    });
  } catch (error) {
